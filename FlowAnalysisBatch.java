...
public class FlowAnalysisBatch {
    ...
    private static void writeHtmlReport(File dir, FlowAnalyzer.AnalysisResult result, long elapsed, String sourcePath) throws Exception {
        ...
        html.append("const graph=document.getElementById('graph');const ctx=graph.getContext('2d');const graphWrap=document.getElementById('graphWrap');const tooltip=document.getElementById('nodeTooltip');\n");
        ...
        html.append("function buildSubset(seed,depthFrom,depthTo,maxNodes){const visited=new Set();const hasSeed=seed&&seed.length>0&&DATA.mapNodes.indexOf(seed)>=0;const backwardLimit=Math.abs(Math.min(0,depthFrom));const forwardLimit=Math.max(0,depthTo);function visitDir(start,isForward,limit){const q=[{n:start,d:0}];const bestDepth=new Map();bestDepth.set(start,0);while(q.length&&visited.size<maxNodes){const cur=q.shift();if(cur.d>limit)continue;if(!visited.has(cur.n))visited.add(cur.n);const next=isForward?(outMap.get(cur.n)||[]):(inMap.get(cur.n)||[]);for(let i=0;i<next.length;i++){const nd=cur.d+1;if(nd>limit)continue;const prev=bestDepth.get(next[i]);if(prev!==undefined&&prev<=nd)continue;bestDepth.set(next[i],nd);q.push({n:next[i],d:nd});if(visited.size+q.length>=maxNodes)break;}}}if(hasSeed){visited.add(seed);if(forwardLimit>0)visitDir(seed,true,forwardLimit);if(backwardLimit>0&&visited.size<maxNodes)visitDir(seed,false,backwardLimit);}else{const q=[];for(let i=0;i<DATA.starters.length&&i<50;i++)q.push({n:DATA.starters[i],d:0});if(q.length===0&&DATA.mapNodes.length>0)q.push({n:DATA.mapNodes[0],d:0});while(q.length&&visited.size<maxNodes){const cur=q.shift();if(visited.has(cur.n)||cur.d>forwardLimit)continue;visited.add(cur.n);const outs=outMap.get(cur.n)||[];for(let i=0;i<outs.length;i++){if(!visited.has(outs[i]))q.push({n:outs[i],d:cur.d+1});if(visited.size+q.length>=maxNodes)break;}}}const nodes=[...visited];const nodeSet=new Set(nodes);const edges=[];for(let i=0;i<DATA.mapEdges.length;i++){const e=DATA.mapEdges[i];if(nodeSet.has(e.source)&&nodeSet.has(e.target))edges.push(e);}return {nodes,edges};}\n");
        html.append("let current={nodes:[],edges:[],pos:new Map()};let view={offsetX:40,offsetY:40,scale:1};let dragNode='';let isPanning=false;let lastX=0;let lastY=0;\n");
        ...
        html.append("function draw(){ctx.clearRect(0,0,graph.width,graph.height);ctx.strokeStyle='rgba(56,189,248,0.25)';for(let i=0;i<current.edges.length;i++){const e=current.edges[i];const a=toScreen(current.pos.get(e.source));const b=toScreen(current.pos.get(e.target));ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}current.nodes.forEach(name=>{const p=toScreen(current.pos.get(name));const isStart=DATA.starters.indexOf(name)>=0;const isFinal=DATA.finals.indexOf(name)>=0;ctx.fillStyle=isStart?'#34d399':(isFinal?'#f59e0b':'#93c5fd');ctx.beginPath();ctx.arc(p.x,p.y,3.8,0,Math.PI*2);ctx.fill();});}\n");
        html.append("function findNode(mx,my){for(let i=current.nodes.length-1;i>=0;i--){const name=current.nodes[i];const p=toScreen(current.pos.get(name));const dx=mx-p.x;const dy=my-p.y;if((dx*dx+dy*dy)<=64)return name;}return ''; }\n");
        ...
        html.append("graph.addEventListener('mousemove',ev=>{const r=graph.getBoundingClientRect();const mx=ev.clientX-r.left;const my=ev.clientY-r.top;if(dragNode){const p=current.pos.get(dragNode);p.x+=(mx-lastX)/view.scale;p.y+=(my-lastY)/view.scale;lastX=mx;lastY=my;draw();showTooltip(dragNode,mx,my);return;}if(isPanning){view.offsetX+=mx-lastX;view.offsetY+=my-lastY;lastX=mx;lastY=my;draw();return;}const hover=findNode(mx,my);showTooltip(hover,mx,my);});\n");
